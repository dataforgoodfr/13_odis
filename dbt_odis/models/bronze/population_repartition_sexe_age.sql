{{ config(
    ...
    alias = 'vw_population_repartition_sexe_age'
    )
}}

with repartition_sexe_age as (
    select
        id as id,
    ...
    from {{ source('bronze', 'population_repartition_sexe_age') }}  
)

select * from repartition_sexe_age