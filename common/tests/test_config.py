...
def test_load_config_with_dictionary():
    # given
    yaml_config = """
    APIs:
        INSEE.Metadonnees:
    ...
    # when
    with patch("builtins.open", mocked_open_function):
        result = load_config("", response_model=DataSourceModel)  # any path will do

    # then
    assert result.get_model("geographical_references.regions") is not None
    assert result.get_model("geographical_references.regions").dictionary is not None
    assert (
        result.get_model("geographical_references.regions").dictionary["key1"]
        == "value1"
    )
    assert (
        result.get_model("geographical_references.regions").dictionary["key2"]
        == "value2"
    )
