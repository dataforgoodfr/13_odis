...
def test_create_or_overwrite_table_comment_on_table_with_quotes(
    pg_con: psycopg2.extensions.connection, pg_settings: dict, mocker
):
    """case where the comment on table contains quotes"""
    # given
    table_description = "test table d'escription"
    config = DataSourceModel(
        **{
            "APIs": {
                "api1": {
...
def test_create_or_overwrite_table_comment_on_columns_with_quotes(
    pg_con: psycopg2.extensions.connection, pg_settings: dict, mocker
):
