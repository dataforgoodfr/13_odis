...
def add_commune_to_map(df, geometry, fg, style):
    ...
    commune_json = flm.GeoJson(data=df, style_function=lambda feature: styling_communes(feature=feature, style=style))
...
def dataset_callback(container, index=None):
    """
    Example callback that only runs when lazy_expander is expanded.
    """
    if container is not None:
        with container:
            st.write("Lazy-loaded content goes here!")
            st.write(f"Entry data: {index}")  # Or load heavy dataset, images, etc.
    ...
    if st.session_state["processed_gdf"] is not None:
        for index, row in st.session_state["processed_gdf"].head(5).iterrows():
            ...
            lazy_expander(
                title=title,
                key=title,
                on_expand=dataset_callback,
                expanded=False,
                callback_kwargs={"index": index},
            )
